...
namespace Affine
{
    class Graphic : Polyhedron
    {
        ...
        public void DrawGraphic(double step, int left_range, int right_range)
        {
            ...
            for (double x = left_range; x <= right_range; x += step)
            {
                List<Point> currentCurve = new List<Point>();
                for (double y = left_range; y <= right_range; y += step)
                {
                    double z = F(x, y);

                    // отображение координат на данной проекции
                    double _phi = phi * Math.PI / 180;
                    double _psi = psi * Math.PI / 180;
                    double fx = x * Math.Cos(_psi) - (Math.Cos(_phi) * z - Math.Sin(_phi) * y) * Math.Sin(_psi);
                    double fy = y * Math.Cos(_psi) + z * Math.Sin(_phi);
                    // корректировка размера графика
                    double k = 50d;
